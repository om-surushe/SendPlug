...
describe("environment", () => {
  const required = {
    ...
    RECOVERY_ADMIN_PASSWORD: "strong-password-1",
  };

  test("requires database and Redis URLs", () => {
    expect(() => loadEnvironment({ NODE_ENV: "test" })).toThrow("PUBLIC_ORIGIN is required");
  });

  test("parses a complete environment", () => {
    const result = loadEnvironment({ ...required, API_PORT: "3100" });
    expect(result.port).toBe(3100);
    expect(result.databaseUrl).toBe(required.DATABASE_URL);
    expect(result.signupsEnabled).toBe(true);
    expect(result.workos).toBeNull();
  });

  test("rejects incomplete WorkOS configuration", () => {
    expect(() => loadEnvironment({ ...required, WORKOS_CLIENT_ID: "client_test" })).toThrow("WORKOS_REDIRECT_URI is required");
  });
  ...
  test("parses complete WorkOS configuration", () => {
    expect(loadEnvironment({
      ...
      clientId: "client_test", apiKey: "test-key", cookiePassword: "a".repeat(32), redirectUri: "https://app.example.com/workos/callback",
    });
  });
});
