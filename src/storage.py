...
def create_local_identity(email: str, password: str, name: str = "") -> dict[str, str]:
    normalized_email = email.lower().strip()
    display_name = name.strip() or normalized_email.split("@", 1)[0]
    now = _now()
    account_id = f"account_{uuid.uuid4().hex}"
    user_id = f"user_{uuid.uuid4().hex}"
    account_name = f"{display_name}'s workspace"
    with connect() as db:
        if db.execute("SELECT 1 FROM users WHERE lower(email) = ?", (normalized_email,)).fetchone():
        ...
        db.execute(
            """INSERT INTO users
               (id, provider, provider_subject, email, name, password_hash, created_at, last_login_at)
               VALUES (?, 'local', ?, ?, ?, ?, ?, ?)""",
            (user_id, normalized_email, normalized_email, display_name, _password_hash(password), now, now),
        )
        db.execute(
...
def authenticate_local_identity(email: str, password: str) -> Optional[dict[str, str]]:
    normalized_email = email.lower().strip()
    with connect() as db:
        row = db.execute(
            """SELECT u.id AS user_id, u.email, u.name, u.password_hash,
                      m.account_id, m.role, a.name AS account_name
               FROM users u JOIN memberships m ON m.user_id = u.id
               JOIN accounts a ON a.id = m.account_id
               WHERE u.provider = 'local' AND u.provider_subject = ?""",
            (normalized_email,),
        ).fetchone()
        if not row or not row["password_hash"] or not _password_matches(password, row["password_hash"]):
            return None
        db.execute("UPDATE users SET last_login_at = ? WHERE id = ?", (_now(), row["user_id"]))
        return {key: row[key] for key in ("user_id", "email", "name", "account_id", "account_name", "role")}
