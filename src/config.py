...
class Config:
    ...
    SEND_BURST_LIMIT = int(os.getenv("SEND_BURST_LIMIT", "10"))
    SEND_BURST_WINDOW_SECONDS = int(os.getenv("SEND_BURST_WINDOW_SECONDS", "60"))
    ...
    def validate(cls) -> None:
        ...
        if cls.ENVIRONMENT == "production":
            placeholders = {"changeme", "generate-a-long-random-password"}
            configured_passwords = {cls.ADMIN_PASSWORD, cls.SMTP_AUTH_PASSWORD or "", cls.FLOWER_PASSWORD}
            if configured_passwords & placeholders:
                raise RuntimeError("Replace documented placeholder passwords before starting production")
            if not cls.FLOWER_USER or not cls.FLOWER_PASSWORD:
                raise RuntimeError("FLOWER_USER and FLOWER_PASSWORD are required in production")
        ...
        if cls.SEND_BURST_LIMIT < 0 or cls.SEND_BURST_WINDOW_SECONDS < 1:
            raise RuntimeError("SEND_BURST_LIMIT must be non-negative and SEND_BURST_WINDOW_SECONDS positive")
        logger.info(
